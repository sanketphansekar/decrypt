...
<body style="background-color:#E4E4E4">
	...
	if((int)$decrypted >= 17){
		echo '<h4 style="margin-left: 20px;" class="text-danger">Last Challenge : '. $_SESSION['previous'] .'</h4>';
			...
			<h3 style="/* Padding-left:20%; */ padding-top:20px;" class="text-success">So I was in my Office suddenly i had the urge to take a dip in the swimming Pool. I went to a colleague, who recently returned from a holiday in Italy. I told him “Cover me, Mate!!” , He promptly replied “Roger That!!”</h3>
			<form style="padding-left:20%;/* width: 60%; */" action="kappa.php" method="post">
				<div style="bottom:-350px;margin-top: 35px;margin-left: 100px;">
					<input type="text" class="form-control" name="iota" style="border:1px solid;width: 60%;"><br>
					<input type="submit" class="btn btn-primary btn-lg" value="SUBMIT" style="margin-left: -40%;">
				</div>
			</form>
			</div>';
	}	
	elseif(isset($_POST["theta"]))
	{
		//$x = str_replace(' ', '', $_POST["answer1"]);		
		$x = strtolower($_POST["theta"]);
		if(strpos($x,'google') !== false){			
			session_destroy();
			session_start();
			$string = 17;
