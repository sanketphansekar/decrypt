...
<body style="background-color:#E4E4E4">
	...
	if((int)$decrypted >= 9){
		echo '<h4 style="margin-left: 20px;" class="text-danger">Last Challenge : '. $_SESSION['previous'] .'</h4>';
			...
			<h3 style="Padding-left:20%; padding-top:20px;" class="text-success">Guess the celebrity!!!</h3>
			<img src="images/quiz/beta.jpg" class="img-thumbnail" alt="sorry" style="margin-left:130px; width:800px; height:300px; margin-top:10px;">
			<form style="padding-left:20%;" action="gamma.php" method="post">
				<div style="bottom:-350px; margin-top:10%;">
					<input type="text" class="form-control" name="beta" style="border:1px solid; width:60%"><br>
					<input type="submit" class="btn btn-primary btn-lg" value="SUBMIT" style="margin-left:25%">
				</div>
			</form>
			</div>';
	}	
	elseif(isset($_POST["alpha"]))
	{
		//$x = str_replace(' ', '', $_POST["answer1"]);		
		$x = strtolower($_POST["alpha"]);
		if(strpos($x,'translate') !== false){			
			session_destroy();
			session_start();
			$string = 9;
