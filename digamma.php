...
<body style="background-color:#E4E4E4">
	...
	if((int)$decrypted >= 13){
		echo '<h4 style="margin-left: 20px;" class="text-danger">Last Challenge : '. $_SESSION['previous'] .'</h4>';
			...
			<h3 style="Padding-left:20%; padding-top:20px;margin-left: -30%;">X L C D ?</h3>
			<form style="padding-left:20%;" action="zeta.php" method="post">
				<div style="bottom:-350px;">
					<input type="text" class="form-control" name="digamma" style="border:1px solid; width:60%;"><br>
					<input type="submit" class="btn btn-primary btn-lg" value="SUBMIT" style="margin-left: -40%;">
				</div>
			</form>
			</div>';
	}	
	elseif(isset($_POST["epsilon"]))
	{
		//$x = str_replace(' ', '', $_POST["answer1"]);		
		$x = strtolower($_POST["epsilon"]);
		if(strpos($x,'ubuntu') !== false){			
			session_destroy();
			session_start();
			$string = 13;
