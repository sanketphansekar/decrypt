...
<body style="background-color:#E4E4E4">
	...
	if((int)$decrypted >= 12){
		echo '<h4 style="margin-left: 20px;" class="text-danger">Last Challenge : '. $_SESSION['previous'] .'</h4>';
			...
			<h3 style="margin-left: -9%; margin-bottom: 50px;padding-top:20px;" class="text-success">Who AM I ?</h3>
			<form style="padding-left:20%;" action="digamma.php" method="post">
				<div style="bottom:-350px;">
					<input type="text" class="form-control" name="epsilon" style="border:1px solid; width:60%"><br>
					<input type="submit" class="btn btn-primary btn-lg" value="SUBMIT" style="margin-left: -35%;">
				</div>
			</form>
			</div>';
	}	
	elseif(isset($_POST["delta"]))
	{
		...
		$x = strtolower($_POST["delta"]);
		if(strpos($x,'marmot') !== false){			
			session_destroy();
			session_start();
			$string = 12;
